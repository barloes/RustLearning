...
impl WsChatServer {
    ...
    fn send_chat_message(
        ...
        //checking the list in the room data and adding all of them 
        for (id, client) in room.drain() {
            println!("aa");
            if client.do_send(ChatMessage(msg.to_owned())).is_ok() {
                println!("bb");

                //what is add client to room
...
impl Handler<SendMessage> for WsChatServer {
    ...
    fn handle(&mut self, msg: SendMessage, _ctx: &mut Self::Context) {
        println!("im at send message handler");
        let SendMessage(room_name, id, msg) = msg;

        println!("{:?}",self.send_chat_message(&room_name, &msg, id));

        //return the rooms with the recipients
